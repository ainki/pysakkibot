...
//jos ei ole
        ...
        if (valinta.includes('/') && numMaara(pyskoodi) === 4 && !pyskoodi.trim().includes(" ")) {
          ...
          if (!isNaN(pyskoodi)) pyskoodi = "H" + pyskoodi;
            ...
            // Funktion kutsuminen
            return valintafunktio(msg.chat.id, pyskoodi, viimekomennot, maara);
        ...
        if (valinta.includes("/") && numMaara(pyskoodi) === 0) {
            ...
            return  bot.sendMessage(msg.chat.id, `Virheellinen haku. Pysäkkikoodeissa on oltava 4 numeroa sekä mahdollinen kuntaetuliite`, {ask: 'ask/valinta'});
