...
function reitti(msg) {
  ...
  bot.on(['location'], (msg) => {
    for (var i = 0; i < lahtopaikat.length; i++) {
      if (lahtopaikat[i].UserId === msg.from.id ) {
        return;
      }
    }
    console.log("msg.location reitti",msg.location);
  ...
  function reittienListaus(msg, lahtopaikka, maaranpaa, data) {
    ...
    for (let i = 0; i < legs.length; i++) {
      ...
      for (let x = 0; x < mode.length; x++) {
        ...
        let lait;
        let testi = JSON.parse(JSON.stringify(data.plan.itineraries[i].legs[x].to.stop).replace("null",'"N/A"'));
         if (testi.platformCode !== "N/A") {
                  lait = testi.platformCode;
        }
