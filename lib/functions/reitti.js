...
// tarkistetaan onks kayttajat-arrays
  ...
  if (toiminto === 'L') {
    ...
    const maaranpaaviesti = Object.assign({ ...msg }, { text: [lat, lon], tyyppi: 'sijainti' })
...
function reitinKasittely (msg) {
  ...
  const plan = msg.lahtoaika
    ? `plan(from: {lat: ${latStart}, lon: ${lonStart}}, to: {lat: ${latDest}, lon: ${lonDest}},date:"${date}",time:"${time}", numItineraries: 3) {`
...
function sijaintiLabelHandler (lat, lon) {
  try {
    const osoiteHaku = JSON.parse(httpGet(encodeURI(`https://api.digitransit.fi/geocoding/v1/reverse?point.lat=${lat}&point.lon=${lon}&size=1`)))
...
function tekstiLabelHandler (text, tyyppi, chatId) {
  try {
    const startingCords = JSON.parse(httpGet(encodeURI(`https://api.digitransit.fi/geocoding/v1/search?text=${text}&size=1&lang=fi&boundary.rect.min_lat=59.9&boundary.rect.max_lat=60.45&boundary.rect.min_lon=24.0&boundary.rect.max_lon=25.5`)))
...
function lisaaKayttajalle (tyyppi, label, lat, lon, chatId) {
  for (let i = 0; i < kayttajat.length; i++) {
    if (kayttajat[i].userId === chatId) kayttajat[i][tyyppi] = { label, lat, lon }
  }
}
