...
function reitinKasittely(msg) {
    ...
    const index = kayttajat.map(i => i.userId).indexOf(msg.from.id);
    ...
    const maaranpaa =  kayttajat[index].maaranpaa;
      maaranpaa ? console.info('Määränpää OK, käsitellään reittiä...',maaranpaa) : console.error("määränpää puuttuu!");
  ...
  function reittienListaus(msg, lahtopaikka, maaranpaa, data) {
    ...
    for (let i = 0; i < legs.length; i++) {
        ...
        for (let x = 0; x < mode.length; x++) {
            ...
            if (x>0 && x < data.plan.itineraries[i].legs.length-1 && data.plan.itineraries[i].legs[x+1].mode !== 'WALK' && data.plan.itineraries[i].legs[x+1].startTime - legEndTime >= 240000) {
              // odotusaika miillisekuntteina
              const odotusaikams = data.plan.itineraries[i].legs[x+1].startTime - legEndTime;
              if (odotusaikams < 3540000) {
                //jos odotus alle 59min
                odotusaika = Math.floor((odotusaikams / (1000 * 60)) % 60)+" min";

              }else if (odotusaikams > 3540000 && odotusaikams < 3660000) {
                // jos suunnillee tunti
                odotusaika = "1h";
              }else {
                //jos yli tunti
                odotusaika = Math.floor((odotusaikams / (1000 * 60)) % 60)+":"+Math.floor((odotusaikams / (1000 * 60 * 60)) % 24)+" h";
              }
            }
