    ...
    .then(function (data) {
      ...
      if (vastaus == tyhjavastaus) {
        ...
        for (i = 0; i < pysakit.length+1; i++) {
          ...
          } else if(laiturit[i] !== null && koodit[i] !== undefined && koodit[i] !== null) {
        ...
        if (pysakit.length === 1 && pysakit[0].toLowerCase() === viesti) {
    ...
    .then(function (data) {
      ...
      if (asetus == 1) {
        ...
        console.log('[info] Lähdöt lähetetty1')
          lahetaAsemanSijainti(chatId, data.stops[0].lat, data.stops[0].lon)
