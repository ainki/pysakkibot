...
bot.on('ask.pysnimi', msg => {
  ...
  index = viestinKoodit.findIndex(e => e.uid === msg.from.id);
...
async function pysakkihaku(chatId, viesti) {
  ...
  if (!Object.keys(data.stops).length) {
      ...
      viestinKoodit[viestinKoodit.findIndex(e => e.uid === chatId)].koodit = koodit;
...
function valintafunktio(chatId, valinta) {
 ...
 .then(function (data) {
   ...
   if (viestinKoodit && !viestinKoodit[viestinKoodit.findIndex(e => e.uid === chatId)].koodit.includes(valintavastaus) && numMaara(valintavastaus) === 4) {
