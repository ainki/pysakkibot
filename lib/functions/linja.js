...
function maaranpaat(chatId, viesti) {
  ...
  var tanaan = date.format(new Date(), 'YYYYMMDD')
  ...
  .then(function (data) {
    ...
    for (let i = 0; i < shortNames.length; i++  ) {
      ...
      if (linjatunnus == viesti) {
        ...
        for (let i = 0; i < maaranpaat.length; i++  ) {
          ...
          if (tripsForDate[i] == '') {
            ...
            maaranpaalista += JSON.stringify(vaihtoehtoNumero) + ' - ' + maaranpaat[i] + '\n'
          }
        } break;
