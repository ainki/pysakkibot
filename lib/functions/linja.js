...
bot.on('ask.maaranpaavalinta', msg => {
  ...
  if (msg.text.includes("/")) {
    ...
    linjaVarasto = linjaVarasto.filter(e => e.userId === msg.chat.id);
  } else {
    // Action typing
    bot.sendAction(msg.from.id, 'typing');
    // Jokaiselle userId:lle
    const index = linjaVarasto.findIndex(e => e.userId === msg.chat.id);

      if (userId[index] == msg.chat.id) {
...
//userId:n indexi
  const index = maaranpaaVarasto.findIndex(e => e.id === chatId);
  ...
  if (valinta.includes('/')) {
      ...
      if (index !== -1) {
        ...
        maaranpaaVarasto.splice(index, 1);
        pysakit.splice(index, 1);
  ...
  let pysakinId;
      ...
      if (index !== -1) {
        const pysakitUIDindex = pysakit.findIndex(e => e.id === chatId);
          if (pysakitUIDindex !== -1) {
            const pysakinKoodit = pysakitKooditJaIdt[pysakitUIDindex].stopCodes;
            const pysakinNimet = pysakitKooditJaIdt[pysakitUIDindex].stopNames;
            const pysakinIdt = pysakitKooditJaIdt[pysakitUIDindex].stopIds;
