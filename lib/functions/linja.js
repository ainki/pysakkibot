...
const nyt = new Date();
  ...
  if (msg.text.includes("/")) {
    ...
    // linjaVarasto = linjaVarasto.filter(e => e.uid === msg.chat.id);
  } else {
    // Action typing
    bot.sendAction(msg.from.id, 'typing');
    ...
    const valintaNumerot = kayttajat[index].numerot;
      if (kayttajat[index] !== -1) {
        for (let x = 0; x < valintaNumerot.length; x++  ) {
          console.log("valintaNumerot[x]",valintaNumerot[x]);
...
function pysakkienhaku(chatId, linjanId) {
  ...
  .then(function (data) {
    ...
    const linjanId = data.pattern.code;
  ...
  let pysakinId;
      ...
      if (index !== -1) {
          if (index !== -1) {
            ...
            const pysakinIdt = pysakitKooditJaIdt.stopIds;
