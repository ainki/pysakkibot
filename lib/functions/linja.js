...
function linja(chatId, viesti) {
    // Jos saadaan vain /lina, kysytään ask.linjatunnuksella linjaa
    if (viesti == '/linja') {
            ...
            if (userId[i] == chatId) {
                ...
                for (x = 0; x < valintaNumerot.length; x += 1) {
                    ...
                    if (valintaNumero == valinta) {
                        console.log('Valinta: ' + valintaNumero)
...
function pysakkihaku(chatId, code) {
    ...
    return request(digiAPI, query)
        .then(function (data) {
            ...
            var element = {};
            element.userId = chatId
            element.stopNames = pysakkinimet
            element.stopCodes = pysakkikooditArray
...
function aikataulut(chatId, valinta) {
    ...
    var linjanKoodi = jp.query(element, '$..koodi')

    // Datan haku pysakit
    var kaikkipysakit = jp.query(pysakit, '$..element')
    var pysakkienNimet = jp.query(kaikkipysakit, '$..stopNames.*')
    var pysakkienKoodit = jp.query(kaikkipysakit, '$..stopCodes.*')
    ...
    for (i = 0; i < userId.length; i += 1) {
        ...
        if (userId[i] == chatId) {
            ...
            // maaranpaaVarasto.splice(i, 1)
            // pysakit.splice(i, 1)
        } for (x = 0; x < pysakkinimet.length; x += 1) {
            pysakinKoodi = pysakkienKoodit[x]
            if (pysakkienNimet[x] == valinta) {
                // Pysäyttää loopin
                break
            }
        }
    ...
    console.log('Pysakin koodi: ' + pysakinKoodi)
