 ...
 viestiLista =  await viestiListaAlku.concat(viestiLista);
    ...
    if (viestiLista.length >= 1) {
      ...
      let  nappaimisto = chunkArray(nappaimistoLista, 5);
        ...
        bot.sendMessage(chatId, `Etsit liityntäpysäköintiä ${viesti}.\nValitse alla olevista vaihtoehdoista oikea liityntäpysäköintipaikka!\n\n${viestiLista}\n`, {replyMarkup, ask: 'liityntakoodi'})
    ...
    return request(digiAPI, querynimiHaku)
            .then(function (data) {
                ...
                if (carParks == '') {
                    ...
                    realtime[0] ? bot.sendMessage(chatId, 'Liityntäpysäköinti ' + carParksName + ' - ' + carParksId + '\n\nPaikkoja vapaana: ' + spacesAvailable + '/' + maxCapacity, {ask: 'liityntakoodi'}) : bot.sendMessage(chatId, 'Liityntäpysäköinti '+carParksName + ' - ' + carParksId + '\n\nPaikkoja vapaana: ?/' + maxCapacity, {ask: 'liityntakoodi'})
