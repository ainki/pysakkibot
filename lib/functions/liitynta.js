...
function liityntaHaku (chatId, viesti) {
  ...
  return request(digiAPI, querynimiHaku)
    .then(function (data) {
      ...
      if (carParks[0][0] === null) {
        // Jos liityntaa ei ole olemassa

        bot.sendMessage(chatId, `Liityntäpysäköintipaikkaa <i>${viesti}</i> ei löydy.\nKokeile uudestaan!`, { ask: 'liityntakoodi', parseMode: 'html' })
        return console.info(' liityntaa ei löydy.')
      } else {
        // Jatkaa rakentamalla viestin
        realtime[0] ? bot.sendMessage(chatId, 'Liityntäpysäköinti\n<b>' + carParksName + '</b> - ' + carParksId + '\n\nPysäköintipaikkoja vapaana (' + spacesAvailable + '/' + maxCapacity + ')', { ask: 'liityntakoodi', parseMode: 'html' }) : bot.sendMessage(chatId, 'Liityntäpysäköinti\n' + carParksName + ' - ' + carParksId + '\n\nPysäköintipaikkoja vapaana (?/' + maxCapacity + ')', { ask: 'liityntakoodi' })
