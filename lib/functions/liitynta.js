...
function liitynta(chatId, viesti) {
  ...
  if (viesti == "/liitynta") {
    ...
    return console.log('[info]  Kysytty liitynnän koodia.')
  } else {
...
async function nimiHaku(chatId, viesti) {
  ...
  for (var i = 0; i < kooditJaNimet.liityntalista.length; i++) {
    ...
    if (kooditJaNimet.liityntalista[i].name.slice(0,viesti.length).toLowerCase() === viesti) {
  ...
  if (viestiLista.length === 1 && loyty) {
    liityntaHaku(chatId, objviestiLista[0].id)
...
console.log("realtime?",realtime[0], typeof realtime);
    ...
    if (carParks == '') {
      ...
      return console.log('[info]  liityntaa ei löydy.')
    } else {
...
realtime[0] ? bot.sendMessage(chatId, carParksName + ' - ' + carParksId + '\n\nPaikkoja vapaana: ' + spacesAvailable +'/'+maxCapacity, { ask: 'liityntakoodi' }) : bot.sendMessage(chatId, carParksName + ' - ' + carParksId + '\n\nPaikkoja vapaana: ?/'+maxCapacity, { ask: 'liityntakoodi' })
      ...
      // lahetaliitynnanSijainti(chatId, carParksLat, carParksLon)
      // console.log(nappaimisto);
      return console.log('[info]  liitynnän tiedot lähetetty.')
    }
  })
...
console.log("tarkistaKoodi");
  ...
  if (isNaN(viesti)) {
    ...
    bot.sendMessage(chatId, `liityntäpysäköintipaikkaa ${viesti} ei löydy.\nKokeile uudestaan!`, { ask: 'liityntakoodi' })
    return console.log('[info]  Vain nollia liityntakoodissa.')
    ...
    return console.log('[info]  Liian pitkä liityntakoodi.')
  } else {
