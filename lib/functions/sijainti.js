      ...
      if (vastaus === tyhjavastausLOC) {
        ...
        pysakit = nodehaku.reduce((a, e) => {
          ...
          if (stoptimes) {
            ...
            headsign = stoptimes.headsign ? stoptimes.headsign : 'tuntematon'
            // poistetaan turha "via" jos on
            if (headsign.slice(-4) === ' via') {
              headsign = headsign.slice(0, -4)
            }

            pysahdykset.push({ koodi, pysakkinimi, lahtoaika: stoptimes.realtimeDeparture, distance, realtime: stoptimes.realtime, lahtopaiva: stoptimes.serviceDay, shortname, headsign })
            // jos ei oo lisätty jo a:han, jotta lisättäis vaan uniikkei
            if (!a.some(x => x.koodi === koodi)) a.push({ koodi, nimi: pysakkinimi, distance, yksittaisetlahdot: [] })
          }
        ...
        for (let i = 0; i < pysahdykset.length; i++) {
          ...
          const yksittainenlahto = pysahdykset[i].realtime ? '<b>' + pysahdykset[i].lahtoaika + '</b>•  <b>' + pysahdykset[i].shortname + '</b> ' + pysahdykset[i].headsign : '<b>' + pysahdykset[i].lahtoaika + '</b>    <b>' + pysahdykset[i].shortname + '</b> ' + pysahdykset[i].headsign
