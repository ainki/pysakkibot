    ...
    return request(digiAPI, querygetlocation)
            .then(function (data) {
                ...
                if (vastaus == tyhjavastausLOC) {
                    bot.sendMessage(msg.chat.id, `Läheltäsi ei valitettavastai löydy pysäkkejä.`);
                    ...
                    for (var i = 0; i < pysakit.length; i++) {
                        if (/\d/.test(pysakit[i].koodi)) {
                            nappainvaihtoehdot.push(bot.inlineButton(pysakit[i].koodi, {callback: "K,/" + pysakit[i].koodi}));
                        }
                        //lisätään lähdöt
                        lahdot += "\n\n<b>" + pysakit[i].nimi + "</b> - " + pysakit[i].koodi + " → " + pysakit[i].distance + "m" + "\n" + pysakit[i].yksittaisetlahdot.join("\n");
                    }

                    //const replyMarkup = bot.keyboard(chunkArray(nappainvaihtoehdot, 5), {ask: "sijaintivalinta", resize: true });
