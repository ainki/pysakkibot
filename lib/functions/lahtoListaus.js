...
function lahtoListaus(data, asetus) {
  ...
  for (let i = 0; i < Haku.length; i++  ) {
    ...
    if (Haku[i]) {
      ...
      if (lahdotPysakeilta === "") {
        if (platformCode[i] === null) {
          ...
          lahdotPysakeilta = 'Lähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - Lait. ' + platformCode[i] + '\n\n'
        }
      } else {
        if (platformCode[i] == null) {
          lahdotPysakeilta += '\nLähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + '\n\n'
        } else {
          lahdotPysakeilta += '\nLähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - Lait. ' + platformCode[i] + '\n\n'
        }
    ...
    // HACK
    if (linjaTunnus.length === 0 && pysakkienKoodit[0] !== "0070") {
      return "Ei lähtöjä pysäkiltä " +pysakkienNimet[0]+" - "+pysakkienKoodit[0]+ "."
    }
      ...
      if (Haku[i]) {
        ...
        if (realtimeTila[x] == 'SCHEDULED') {
          ...
          if (pickupType[x] == 'SCHEDULED') {
            if (alert == "") {
              var yksittainenlahto = departuretimeshort + "‏‏‎     " + linjaTunnus[x] + " " + maaranpaat[x] + "\n";
            } else {
              var yksittainenlahto = departuretimeshort + "‏‏‎     " + linjaTunnus[x] + " " + maaranpaat[x] + "  ⚠️\n";
              poikkeus = true
            }
