 ...
 viestiOK = true;
      ...
      if (lahdotPysakeilta === "") {
        if (laituriKoodit[i] === null) {
          lahdotPysakeilta = 'Lähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - '+ vyohyke[i] +'\n\n';
        } else {
          lahdotPysakeilta = 'Lähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - '+ vyohyke[i] + ' - Lait. ' + laituriKoodit[i] + '\n\n';
        }
      } else {
        if (laituriKoodit[i] == null) {
          lahdotPysakeilta += '\nLähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - '+ vyohyke[i] + '\n\n';
        } else {
          lahdotPysakeilta += '\nLähdöt pysäkiltä ' + pysakkienNimet[i] + ' - ' + pysakkienKoodit[i] + ' - '+ vyohyke[i] + ' - Lait. ' + laituriKoodit[i] + '\n\n';
        }
      }
...
if(lahtopaiva[x].setHours(0,0,0,0) > todaysDate.setHours(0,0,0,0)) {
lahtoaika = lahtoaika + "⁺¹";
}
      ...
      let splitattu = lahtoaika.split(":");
        ...
        if (realtimeTilat[x] == 'SCHEDULED') {
          ...
          if (pickupType[x] == 'SCHEDULED') {
            if (alert == "") {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " " + maaranpaat[x] + "";
            } else {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " " + maaranpaat[x] + "  ℹ️";
              poikkeus = true
            }

          } else if (pickupType[x] == 'NONE' && dropoffType[x] == 'SCHEDULED' && maaranpaat[x] !== null) {
            if (alert == "") {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " ㊀ Vain poistuminen";
            } else {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " ㊀ Vain poistuminen  ℹ️";
              poikkeus = true
            }

          } else if (pickupType[x] == 'NONE' && maaranpaat[x] !== null) {
            var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " ⇥ Saapuu / Päätepysäkki";
          } else if (pickupType[x] == 'NONE') {
            if (alert == "") {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " ⇥ Saapuu / Päätepysäkki";
            } else {
              var yksittainenlahto = lahtoaika + "‏‏‎     " + linjaTunnukset[x] + " ⇥ Saapuu / Päätepysäkki  ℹ️";
              poikkeus = true
            }
          }
          ...
          var yksittainenlahto = lahtoaika + "×‏‏‎   " + linjaTunnukset[x] + " Peruttu";
