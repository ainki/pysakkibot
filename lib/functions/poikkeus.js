      ...
      kaikkilista = alerts.reduce((a,e) => {
        ...
        if (e.alertSeverityLevel !== 'INFO') {
          //tarkistetaan onko uniikki
            ...
            // Poistaa linkin
          let  splitattu = viesti.includes("Info") ? viesti.split("Info") : viesti.split("HSL");
            splitattu[0].trim();
          a.push(splitattu[0].trim()); //lisätään kerryttäjää
        }
        return a;
      },[]);
...
function poikkeuksetlinjalle(chatId, text) {
  ...
  .then(function (data) {
      ...
      if (e.route && e.route.shortName === text){
        ...
        const  splitattu = poikkeus.includes("Info") ? poikkeus.split("Info") : poikkeus.split("HSL"); // splitataan infon tai HSL:n kohalta, jottei tulis linkkii
        a.push(splitattu[0].trim());
      }
        return a;
      },[]);
