...
function kaikkipoikkeukset(chatId) {
  ...
  .then(function (data) {
    ...
    if (!alerts.length) {
      ...
      return kaikkilista.length ? bot.sendMessage(chatId, '<b>Tiedossa olevat poikkeukset</b>\n\n' + kaikkilista.join("\n\n") + "\n\n" + '<b>Lisätietoa: hsl.fi</b>\n\n<i>Anna linjan tunnus linjakohtaisille poikkeuksille!</i>', { parseMode: 'html',replyMarkup, ask: 'linjapoikkeus'}) : bot.sendMessage(chatId, 'Ei tiedossa olevia häiriöitä tai poikkeuksia.');
    }
  });
}
